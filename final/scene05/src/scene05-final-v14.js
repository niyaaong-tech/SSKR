...
Promise.all([
  ...
  // All curated OSM fragments are retained so adjacent ways reconnect into a coherent web.
  // Opacity, not data deletion, keeps the network subordinate to the gold journey layer.
  const major=roadNetworkFrom(roads,new Set(['motorway','trunk']),{color:0xc9d4ce,width:.78,dayOpacity:.095,sunsetOpacity:.010});
  const primary=roadNetworkFrom(roads,new Set(['primary']),{color:0xaebbb4,width:.55,dayOpacity:.043,sunsetOpacity:.005});
  ...
  finishNode=makeNode(data.finish.position,.26);finishNode.scale.setScalar(.56);
  const fb=finishNode.userData.sprite.userData.baseScale*1.9;finishNode.userData.sprite.userData.baseScale=fb;finishNode.userData.sprite.scale.set(fb,fb,1);nodeGroup.add(finishNode);
...
function play(){
  ...
  // Daylight: the real road web becomes a faint continuous context layer.
  ...
  // Sunset convergence: starts and road context yield, the West Finish owns the climax.
  ...
  tl.to(finishNode.scale,{x:1.92,y:1.92,z:1.92,duration:.56,ease:'power2.out'},9.1);nodeOpacity(tl,finishNode,9.08,{core:1,ring:1,glow:1,duration:.42});
  ...
  tl.to(finishNode.userData.sprite.material,{opacity:.20,duration:.36},10.55).to(finishNode.userData.ring.material,{opacity:.34,duration:.36},10.55);

  // Start node → dawn sun match cut. Remove all presentation chrome before the orb owns the frame.
...
function showStatic(){
  ...
  mainLines.forEach(l=>{setProgress(l,1);l.material.opacity=.80});roadHintNetworks.forEach(n=>n.material.opacity=n.userData.dayOpacity*.78);mergedNetwork.material.opacity=.72;statement.style.opacity='1';statement.style.transform='none';nodeOpacity(gsap.timeline(),finishNode,0,{core:1,ring:.8,glow:.8});finishNode.scale.setScalar(1.5);
}

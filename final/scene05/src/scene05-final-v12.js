...
Promise.all([
  ...
  const major=roadNetworkFrom(roads,new Set(['motorway','trunk']),{color:0x80978f,width:.95,dayOpacity:.12,sunsetOpacity:.025});
  const primary=roadNetworkFrom(roads,new Set(['primary']),{color:0x8b9b93,width:.72,dayOpacity:.065,sunsetOpacity:.012});
...
function play(){
  ...
  // Day: enough real-road context to imply choice, but always below Main/Merged hierarchy.
  ...
  roadHintNetworks.forEach((n,i)=>tl.to(n.material,{opacity:n.userData.dayOpacity,duration:.65,ease:'sine.out'},5.05+i*.12));
  tl.to(mergedNetwork.material,{opacity:.62,duration:.62,ease:'power2.out'},5.5);
  ...
  tl.to(mergedNetwork.material,{opacity:.28,duration:.65},8.0);mainLines.forEach((l,i)=>tl.to(l.material,{opacity:.42,duration:.65},8.0+i*.02));
...
function showStatic(){
  ...
  mainLines.forEach(l=>{setProgress(l,1);l.material.opacity=.82});roadHintNetworks.forEach(n=>n.material.opacity=n.userData.dayOpacity*.7);mergedNetwork.material.opacity=.45;statement.style.opacity='1';statement.style.transform='none';nodeOpacity(gsap.timeline(),finishNode,0,{core:1,ring:.7,glow:.7});finishNode.scale.setScalar(1.25);
}

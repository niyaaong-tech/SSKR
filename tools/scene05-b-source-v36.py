...
# ---------------------------------------------------------------------------
patch('renderer.toneMappingExposure = 1.05;', 'renderer.toneMappingExposure = .96;', 'dawn base exposure')
patch("gsap.set(skyNight, { opacity: .18 });", "gsap.set(skyNight, { opacity: .28 });", 'darker predawn sky')
patch("gsap.set(skyDawn, { opacity: .45 });", "gsap.set(skyDawn, { opacity: .60 });", 'stronger dawn sky')
patch("gsap.set(lightWash, { opacity: .05 });", "gsap.set(lightWash, { opacity: .10 });", 'directional dawn wash')
patch("if (scene.background) scene.background.setRGB(.07, .13, .16);", "if (scene.background) scene.background.setRGB(.035, .075, .105);", 'predawn background')
patch("scene.fog.color.setRGB(.12, .19, .21);", "scene.fog.color.setRGB(.08, .13, .16);", 'predawn fog color')
patch("peninsulaSurface.material.color.setRGB(.72, .78, .76);", "peninsulaSurface.material.color.setRGB(.56, .66, .70);", 'cool dawn land grade')
...
"""  tl.to(scene.background, { r: .07, g: .13, b: .17, duration: 3.0, ease: 'sine.inOut' }, 0)
    .to(scene.fog, { density: .0058, duration: 3.0, ease: 'sine.inOut' }, 0)
    .to(scene.fog.color, { r: .11, g: .17, b: .20, duration: 3.0 }, 0)
    .to(renderer, { toneMappingExposure: .98, duration: 3.0, ease: 'sine.inOut' }, 0);""",
'opening dawn progression'
)

patch(".to(eastGlow, { opacity: .54, duration: 2.5, ease: 'sine.out' }, 3.2)", ".to(eastGlow, { opacity: .74, duration: 2.5, ease: 'sine.out' }, 3.2)", 'east dawn glow')
patch(".to(skyDawn, { opacity: .68, duration: 2.7 }, 3.0)", ".to(skyDawn, { opacity: .90, duration: 2.7 }, 3.0)", 'dawn sky before starts')
...
new_start_lighting = """  // Hold dawn through the Start cascade. The first riders leave under cool ambient
  ...
  if (typeof peninsulaSurface !== 'undefined' && peninsulaSurface) {
    tl.to(peninsulaSurface.material.color, { r: .68, g: .76, b: .79, duration: 3.2, ease: 'sine.inOut' }, 5.4);
  }
  tl.to(scene.background, { r: .11, g: .20, b: .24, duration: 3.2, ease: 'sine.inOut' }, 5.3)
    .to(skyDay, { opacity: .10, duration: 2.6 }, 6.0)
    .to(skyNight, { opacity: .09, duration: 2.8 }, 5.5)
    .to(skyDawn, { opacity: .92, duration: 2.4 }, 5.5)
    .to(eastGlow, { opacity: .72, duration: 2.0 }, 5.8)
    .to(oceanUniforms.uPhase, { value: .68, duration: 3.2, ease: 'sine.inOut' }, 5.5)
    .to(scene.fog, { density: .00535, duration: 3.2 }, 5.5)
    .to(scene.fog.color, { r: .17, g: .24, b: .27, duration: 3.2 }, 5.5)
    .to(renderer, { toneMappingExposure: .99, duration: 3.2, ease: 'sine.inOut' }, 5.5);"""
...
route_lighting = """  // Journey light arc: morning grows into neutral day, then bends warm before Finish.
  ...
  tl.to(scene.background, { r: .27, g: .42, b: .49, duration: 4.8, ease: 'sine.inOut' }, 9.0)
    ...
    .to(eastGlow, { opacity: .10, duration: 4.0, ease: 'sine.inOut' }, 9.5)
    ...
    .to(lightWash, { opacity: .065, duration: 3.8, ease: 'sine.inOut' }, 9.4)
    ...
    .to(westGlow, { opacity: .18, duration: 3.4, ease: 'sine.inOut' }, 15.3)
...
new_finish_lighting = """  // Sunset begins before the last route reaches Finish so arrival already feels like evening.
  ...
  tl.to(scene.background, { r: .32, g: .19, b: .20, duration: 5.0, ease: 'sine.inOut' }, 16.9)
    ...
    .to(westGlow, { opacity: .84, duration: 4.7, ease: 'sine.out' }, 17.1)
    .to(eastGlow, { opacity: .03, duration: 3.0 }, 17.0)
    ...
    .to(lightWash, { opacity: .14, duration: 4.0, ease: 'sine.inOut' }, 17.0)

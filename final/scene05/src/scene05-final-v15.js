...
const statement=$('#statement'),matchOrb=$('#match-orb'),lightWash=$('#light-wash'),sceneMark=$('.scene-mark');
...
const bloom=new UnrealBloomPass(new THREE.Vector2(stage.clientWidth,stage.clientHeight),.7,.34,.76);
...
function makeNode(position,radius=.15){
  ...
  const core=new THREE.Mesh(new THREE.SphereGeometry(radius,20,12),new THREE.MeshBasicMaterial({color:'#ffd166',transparent:true,opacity:0,depthTest:false,depthWrite:false}));
  const ring=new THREE.Mesh(new THREE.RingGeometry(radius*1.45,radius*1.88,36),new THREE.MeshBasicMaterial({color:'#ffc76b',side:THREE.DoubleSide,transparent:true,opacity:0,depthTest:false,depthWrite:false}));
...
function roadNetworkFrom(items,classes,{color,width,dayOpacity,sunsetOpacity}){
  ...
  const geo=new LineSegmentsGeometry();geo.setPositions(flat);
  // Road Hint is a graphic context layer draped over the real terrain. Depth-test off avoids
  // the decimated mesh swallowing short OSM way fragments; very low opacity keeps it subordinate.
  const mat=new LineMaterial({color,linewidth:width,transparent:true,opacity:0,depthTest:false,depthWrite:false});mat.resolution.set(stage.clientWidth,stage.clientHeight);
  const lines=new LineSegments2(geo,mat);lines.computeLineDistances();lines.renderOrder=6;lines.userData={dayOpacity,sunsetOpacity};return lines;
}
...
Promise.all([
  ...
  const major=roadNetworkFrom(roads,new Set(['motorway','trunk']),{color:0xcbd6d0,width:.8,dayOpacity:.065,sunsetOpacity:.006});
  const primary=roadNetworkFrom(roads,new Set(['primary']),{color:0xb4c0ba,width:.56,dayOpacity:.028,sunsetOpacity:.003});
  ...
  finishNode=makeNode(data.finish.position,.27);finishNode.scale.setScalar(.56);
  const fb=finishNode.userData.sprite.userData.baseScale*2.45;finishNode.userData.sprite.userData.baseScale=fb;finishNode.userData.sprite.scale.set(fb,fb,1);nodeGroup.add(finishNode);
...
function play(){
  ...
  gsap.set(stage,{opacity:0});gsap.set(overviewLayer,{opacity:1});gsap.set(overview,{scale:.61,xPercent:0,yPercent:0,opacity:.18});gsap.set(statement,{opacity:0,y:16});gsap.set(sceneMark,{opacity:.35});gsap.set(matchOrb,{opacity:0,scale:0});
  ...
  // Daylight: full real major-road topology reads as a continuous but quiet web.
  ...
  mainLines.forEach((l,i)=>tl.to(l.material,{opacity:.86,duration:.45},5.36+i*.025));
  ...
  // Sunset convergence: starts and road context yield, the West Finish becomes the strongest graphic light.
  ...
  tl.to(finishNode.scale,{x:2.2,y:2.2,z:2.2,duration:.56,ease:'power2.out'},9.1);nodeOpacity(tl,finishNode,9.08,{core:1,ring:1,glow:1,duration:.42});
  ...
  tl.to(finishNode.userData.sprite.material,{opacity:.18,duration:.36},10.55).to(finishNode.userData.ring.material,{opacity:.30,duration:.36},10.55);
  ...
  if(selected){
    ...
    tl.to([statement,sceneMark],{opacity:0,duration:.26},11.55)
      .to(statement,{y:-8,duration:.26},11.55)
...
function showStatic(){
  ...
  mainLines.forEach(l=>{setProgress(l,1);l.material.opacity=.80});roadHintNetworks.forEach(n=>n.material.opacity=n.userData.dayOpacity*.82);mergedNetwork.material.opacity=.72;statement.style.opacity='1';statement.style.transform='none';nodeOpacity(gsap.timeline(),finishNode,0,{core:1,ring:.8,glow:.8});finishNode.scale.setScalar(1.6);
}

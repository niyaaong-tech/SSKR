...
def main():
    ...
    css=(DIST/'scene05-final.css').read_text('utf-8')
    ...
    for old,new in replacements.items():
        html=html.replace(old,new);js=js.replace(old,new)
    html=html.replace('<link rel="stylesheet" href="./scene05-final.css" />',f'<style>{css}</style>')
    ...
    out=DIST/'SSKR_Scene05_Final_v1.8_standalone.html'

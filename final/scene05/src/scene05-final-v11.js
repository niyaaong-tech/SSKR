...
const bloom=new UnrealBloomPass(new THREE.Vector2(stage.clientWidth,stage.clientHeight),.62,.34,.76);
...
const mainLines=[],hintLines=[],seedLines=[],convergenceLines=[],startNodes=new Map();
...
function mergedFrom(items){
  ...
  const mat=new LineMaterial({color:0xffc664,linewidth:2.35,transparent:true,opacity:0,depthTest:true,depthWrite:false});mat.resolution.set(stage.clientWidth,stage.clientHeight);
  const l=new LineSegments2(geo,mat);l.computeLineDistances();l.renderOrder=8;return l;
}
...
Promise.all([
  ...
  for(const r of data.main_routes){
    const hint=lineFromPoints(r.points,{color:0xaab6ad,width:1.15,opacity:0,order:5});hint.userData.id=r.id;hintLines.push(hint);roadHintGroup.add(hint);
    const main=lineFromPoints(r.points,{color:0xffd166,width:3.2,opacity:.98,order:10});main.userData.id=r.id;main.userData.startId=r.start_id;setProgress(main,0);mainLines.push(main);routeGroup.add(main);
    const tail=r.points.slice(r.convergence_from_index);const conv=lineFromPoints(tail,{color:0xffc45d,width:4.4,opacity:0,order:11});conv.userData.id=r.id;setProgress(conv,0);convergenceLines.push(conv);convergenceGroup.add(conv);
  ...
  finishNode=makeNode(data.finish.position,.25);finishNode.scale.setScalar(.56);nodeGroup.add(finishNode);
...
function play(){
  ...
  mainLines.forEach(l=>{l.material.opacity=.98;l.material.linewidth=l.userData.baseWidth;setProgress(l,0)});hintLines.forEach(l=>l.material.opacity=0);seedLines.forEach(l=>{l.material.opacity=0;setProgress(l,0)});convergenceLines.forEach(l=>{l.material.opacity=0;setProgress(l,0)});mergedNetwork.material.opacity=0;
  ...
  finishNode.userData.core.material.opacity=.03;finishNode.userData.ring.material.opacity=.02;finishNode.userData.sprite.material.opacity=.01;
  Object.assign(cam,{x:15,y:52,z:61,tx:0,ty:.1,tz:-2});syncCamera();oceanMat.color.set('#102b3d');oceanMat.opacity=.86;

  // 0.0–0.8: Korea scale in darkness.
  tl.to(overview,{opacity:.25,duration:.5,ease:'sine.out'},.16);

  // 0.8–2.2: the only major camera move, ending with the terrain shifted right so the route story has room.
  ...
  // 2.2–3.5: Dawn starts, north to south. Every Start belongs to the network.
  ...
  // 3.5–5.5: Morning crossing. Five representative journeys grow over real-road topology.
  ...
  // 5.5–7.8: Daylight network. Terrain is deliberately brightest and most legible here.
  ...
  hintLines.forEach((l,i)=>tl.to(l.material,{opacity:.13+(i%2)*.025,duration:.45,ease:'sine.out'},5.15+i*.04));
  tl.to(mergedNetwork.material,{opacity:.66,duration:.62,ease:'power2.out'},5.5);
  ...
  // 7.8–10.2: the day warms westward and the complex network resolves toward Finish.
  ...
  hintLines.forEach(l=>tl.to(l.material,{opacity:.035,duration:.65},7.98));tl.to(mergedNetwork.material,{opacity:.28,duration:.65},8.0);mainLines.forEach((l,i)=>tl.to(l.material,{opacity:.42,duration:.65},8.0+i*.02));
  convergenceLines.forEach((l,i)=>{const p={v:0};tl.to(l.material,{opacity:.98,duration:.24},8.06+i*.055);tl.to(p,{v:1,duration:1.18,onUpdate:()=>setProgress(l,p.v),ease:'power2.inOut'},8.1+i*.065)});
  tl.to(finishNode.scale,{x:1.46,y:1.46,z:1.46,duration:.56,ease:'power2.out'},9.12);nodeOpacity(tl,finishNode,9.1,{core:1,ring:.9,glow:1,duration:.44});
  tl.to(statement,{opacity:1,y:0,duration:.6,ease:'power2.out'},9.33);

  // 10.2–10.55: silence after the Finish climax.
  tl.to({}, {duration:.35},10.2);

  // 10.55–11.6: retrieve one participant's day. Everything else yields.
  hintLines.forEach(l=>tl.to(l.material,{opacity:0,duration:.34},10.5));seedLines.forEach(l=>tl.to(l.material,{opacity:0,duration:.34},10.5));tl.to(mergedNetwork.material,{opacity:0,duration:.36},10.5);convergenceLines.forEach(l=>tl.to(l.material,{opacity:0,duration:.36},10.52));
  ...
  for(const [id,n] of startNodes){const selected=id===personalStart;tl.to(n.userData.sprite.material,{opacity:selected?.85:.035,duration:.4},10.54);tl.to(n.scale,{x:selected?1.34:.72,y:selected?1.34:.72,z:selected?1.34:.72,duration:.4},10.54)}
  if(personalLine)tl.to(personalLine.material,{linewidth:4.4,duration:.35},10.58);

  // 11.6–12.8: Start node becomes the next scene's real dawn sun. No second camera move.
  ...
  if(selected){
    ...
    tl.to(statement,{opacity:0,y:-8,duration:.26},11.55).to(matchOrb,{opacity:1,scale:.9,duration:.32,ease:'power2.out'},11.58).to(matchOrb,{scale:24,opacity:1,duration:1.1,ease:'power2.in'},11.68).to(stage,{opacity:.02,duration:.78,ease:'sine.in'},11.9).to([skySunset,skyDay],{opacity:0,duration:.72},11.9).to(westGlow,{opacity:0,duration:.6},11.9);
  }
  return tl;
}
...
function showStatic(){
  ...
  mainLines.forEach(l=>{setProgress(l,1);l.material.opacity=.82});hintLines.forEach(l=>l.material.opacity=.1);mergedNetwork.material.opacity=.45;statement.style.opacity='1';statement.style.transform='none';nodeOpacity(gsap.timeline(),finishNode,0,{core:1,ring:.7,glow:.7});finishNode.scale.setScalar(1.25);
}

function resize(){const w=stage.clientWidth,h=stage.clientHeight;camera.aspect=w/h;camera.updateProjectionMatrix();renderer.setSize(w,h,false);composer.setSize(w,h);bloom.setSize(w,h);[...mainLines,...hintLines,...seedLines,...convergenceLines].forEach(l=>l.material.resolution.set(w,h));if(mergedNetwork)mergedNetwork.material.resolution.set(w,h)}

...
test('APP catalog has 150 spots, seven starts and one finish without duplicate IDs',()=>{
 assert.equal(catalog.length,158);assert.equal(new Set(catalog.map(p=>p.id)).size,158);
 assert.equal(catalog.filter(p=>p.kind==='spot').length,150);
 assert.equal(catalog.filter(p=>p.kind==='start').length,7);
...
test('APP expansion preserves original Explore records',()=>{assert.equal(base.length,38);for(const p of base){const copy=catalog.find(x=>x.id===p.id);for(const key of Object.keys(p))assert.deepEqual(copy[key],p[key]);}});
test('search ignores whitespace and combines category and region filters',()=>{
 ...
 assert.equal(filterPlaces(catalog,{...all,kind:'start'}).length,7);
});

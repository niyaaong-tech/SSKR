...
# Deterministic reset; generated v2.9 still exposes the baked road texture through
# the roadChoiceNetwork material alias.
reset='  if (roadChoiceNetwork) roadChoiceNetwork.material.opacity = 0;'
patch(reset,'  riderTrailPairs.forEach(p => { p.core.material.opacity = 0; p.glow.material.opacity = 0; setProgress(p.core, 0); setProgress(p.glow, 0); });\n  if (roadChoiceNetwork) roadChoiceNetwork.material.opacity = 0;','rider reset')

...
const terrainMat=new THREE.MeshStandardMaterial({color:'#2d4237',roughness:.9,metalness:.015,emissive:'#06110d',emissiveIntensity:.16});
...
function createRoute(r){
  ...
  for(const f of [.34,.58,.78]){const p=r.points[Math.min(r.points.length-1,Math.floor((r.points.length-1)*f))];const m=new THREE.Mesh(new THREE.SphereGeometry(.085,14,8),new THREE.MeshBasicMaterial({color:'#ffd166',transparent:true,opacity:0,depthWrite:false}));m.position.fromArray(p);m.userData.routeId=r.id;checkpointGroup.add(m)}
  return line;
}
...
}).catch(err=>{console.error(err);document.querySelector('#status').textContent='ASSET LOAD FAILED'});
...
function play(){
  ...
  [...startNodes.values()].forEach((n,i)=>tl.to(n.scale,{x:1,y:1,z:1,duration:.34,ease:'power2.out'},2.35+i*.095));
  ...
  const n=startNodes.get(selected);if(n){
    ...
    const p=n.position;
    tl.to(cam,{x:p.x+17.0,y:20.0,z:p.z+23.0,tx:p.x,ty:p.y+.12,tz:p.z,duration:1.9,onUpdate:syncCamera,ease:'power2.inOut'},10.02);
  }
